...
import { getInnerType, getTypeByName } from './index'
import type { OperationType } from '../flow-types/index'

/**
 * mapQueryToType
 *
 * Maps a graphql query definition to it's eligible fields, and removed some frivolous attributes
 * from the two merged objects, returning only the essential information needed to contruct an
...
export default function (query: any, operationType: OperationType) {
  const mappedTypeName = getInnerType(query.type).name
